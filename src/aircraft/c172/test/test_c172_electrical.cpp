// C172-specific electrical topology tests.
// Loads the C172 electrical.yaml and verifies bus topology, switch gating,
// CB behaviour, failure injection, battery SOC drain, and CAS messages.
...
static GraphSolver makeSolver() {
    ...
    assert(ok && "Failed to load C172 electrical.yaml");
    return s;
}
...
int main() {
    ...
    TEST("Load YAML, verify node/connection counts");
    {
        ...
        // 3 sources + 4 buses + 11 junctions + 22 loads = 40 nodes
        CHECK(topo.nodes.size() == 40);
        CHECK(topo.connections.size() == 39);
    }
    PASS();
