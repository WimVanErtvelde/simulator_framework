...
sim_msgs::msg::FlightModelState JSBSimAdapter::get_state() const
{
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    // Per-bogey actual steering angle (degrees). JSBSim registers
    // fcs/steer-pos-deg[i] for non-castered steerable gear (FGLGear.cpp
    // §844); castered gear uses gear/unit[i]/steering-angle-deg; fixed
    // gear has neither. Fixed bogeys therefore read 0, correctly.
    // Sign is inverted to match the sign flip on fcs/steer-cmd-norm, so
    // rudder_norm > 0 yields wheel_angle_deg > 0 end-to-end.
    std::string steer_prop = "fcs/steer-pos-deg[" + std::to_string(i) + "]";
...
void JSBSimAdapter::apply_payload_command(const sim_msgs::msg::PayloadCommand & cmd)
{
  if (!initialized_ || !exec_) return;
  try {
    for (size_t i = 0; i < cmd.station_indices.size() && i < cmd.weights_lbs.size(); ++i) {
      int idx = cmd.station_indices[i];
      std::string prop = "inertia/pointmass-weight-lbs[" + std::to_string(idx) + "]";
      exec_->SetPropertyValue(prop, static_cast<double>(cmd.weights_lbs[i]));
    }
  } catch (const std::exception & e) {
    log_error(std::string("[JSBSimAdapter] Exception in apply_payload_command: ") + e.what());
  } catch (const std::string & s) {
    log_error("[JSBSimAdapter] JSBSim error in apply_payload_command: " + s);
  } catch (...) {
    log_error("[JSBSimAdapter] Unknown exception in apply_payload_command");
  }
}
